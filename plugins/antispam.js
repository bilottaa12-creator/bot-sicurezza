...
module.exports = {
    ...
    async onMessage(message, ctx) {
        ...
        if (!message.author.bot && (await eModeratoreOAdmin(message.member))) return false;
        ...
        if (troppiMessaggi || troppeRipetizioni) {
            ...
            try {
                ...
                numeroCancellati = messaggiSpammer.size || 1;
            ...
            await inviaLogSicurezza(message.guild,
                `🛡️ **Anti-spam** — <@${userId}> (${message.author.tag}) nel canale <#${message.channel.id}>: ` +
                `${numeroCancellati} messaggi cancellati, timeout ${timeoutOk ? 'applicato' : 'FALLITO'} (${DURATA_TIMEOUT_MIN} min).`
            );
